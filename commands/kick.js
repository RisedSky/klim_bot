    ...
    run: async (call) => {
        ...
        */
                ...
                user_warned.createDM()
                    .then(c => {
                        var embed_kick = new Discord.RichEmbed()
                            .setColor("#E59400")
                            .setDescription(`:warning: Vous avez été kick par ${call.bot.GetUserMention(message.author.id)} - '${message.author.id}'\n\nPour la raison suivante: \`\`\`${call.content.split(call.args[0])[1]}\`\`\` `)
        ...
        let embed_warn = new Discord.RichEmbed()
            .setColor("#E59400")
            .setDescription(`:warning: L'utilisateur ${call.args[0]} a été kick par ${call.bot.GetUserMention(message.author.id)}\n\nPour la raison suivante :\`\`\`${call.content.split(call.args[0])[1]}\`\`\``)
            .setTimestamp()
